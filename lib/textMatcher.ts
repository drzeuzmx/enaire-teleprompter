...
// High-frequency short function words (Spanish + English). These repeat so
// often in normal speech that matching them anywhere in the lookahead window
// causes false skips -- e.g. locking onto a random later "de" or "que". They
// can still confirm normal forward progress, but only when they sit exactly
// at the current cursor position, never as a jump-ahead anchor.
const STOPWORDS = new Set([
  "de", "la", "el", "en", "y", "a", "que", "es", "un", "una", "los", "las",
  "se", "su", "por", "con", "no", "lo", "le", "mi", "tu", "si", "o", "del",
  "al", "te", "me", "nos", "les", "sus", "esta", "este", "esa", "ese", "eso",
  "son", "fue", "ya", "mas", "pero", "como", "cuando", "donde", "porque",
  "para", "sin", "sobre", "entre", "hasta", "desde", "tan", "muy", "eres",
  "the", "a", "an", "of", "to", "in", "on", "is", "it", "be", "or", "at",
  "as", "so", "we", "he", "she", "do", "if", "my", "no", "up", "and", "but",
  "for", "not", "you", "are",
]);
...
export class TextMatcher {
  ...
  consumeTranscript(transcript: string): number {
    ...
    const startCursor = this.currentIndex;
    let cursor = this.currentIndex;

    for (const spoken of spokenWords) {
      const isStopword = STOPWORDS.has(spoken);
      // Common short words are too ambiguous to anchor a jump-ahead: only
      // let them confirm the very next word. Content words keep the full
      // lookahead window so we can still catch up after a missed word.
      const windowEnd = isStopword
        ? Math.min(this.words.length, cursor + 1)
        : Math.min(this.words.length, cursor + this.lookahead);
    ...
    // Guard against a single burst (e.g. a long "final" result some mobile
    // browsers emit instead of frequent interim ones) throwing the cursor
    // forward too far in one update.
    const maxAdvance = Math.max(this.lookahead * 2, 6);
    cursor = Math.min(cursor, startCursor + maxAdvance);
